<script lang="ts">
    ...
    import type { AlbumResult, TicketResult } from "../collage.svelte.js";
...
<div class="res">
    ...
    {#if collageStore.selectedCollage}
        <p style="color: aquamarine; text-align: center;">Adding album(s) to collage "{collageStore.selectedCollage}"</p>
        {#if submittingTicket}
        ...
        {/if}
    {:else}
        <p>You must create and select a collage before you can search for albums.</p>
    {/if} <!-- #if not selected list-->



</div>

<!-- delete collage modal -->
