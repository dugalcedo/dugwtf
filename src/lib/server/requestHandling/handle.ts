...
import { getUserOrNull, getUserOrThrow } from "../serverUtils/getUser";
...
type DRH_Context<S extends ZodObject> = RequestEvent & {
  ...
  getRequiredUser: () => Promise<DbUser>;
  getOptionalUser: () => Promise<DbUser | null>;
};
...
type DRH_Handler<S extends ZodObject> = (
  drhCtx: DRH_Context<S>,
) => DRH_Result | Promise<DRH_Result>;
...
export const drhHandle = <S extends ZodObject>(
  init: DRH_Init<S>,
): RequestHandler => {
  return async (ctx) => {
    // LOG HEAD
    console.log(
      `${timestamp()} NEW REQUEST [${ctx.request.method} ${ctx.request.url}]`,
    );
    ...
    try {
      ...
      const result = await init.handler({
        ...
        keepLoggedIn(user) {
          ...
          ctx.cookies.set("dugwtf-token", token, {
            path: "/",
            httpOnly: true,
            maxAge: 30 * 24 * 60 * 60,
          });
        ...
        async getRequiredUser() {
          return await getUserOrThrow(ctx);
        },
  ...
  switch (err.code) {
    ...
    case "23503":
      return {
        ...base,
        status: 400,
        clientMsg: "referenced record does not exist",
      };
    case "23502":
...
const validateBody = (body: any, schema: ZodObject) => {
  try {
    ...
    const clientMsg =
      zodErr.issues?.map((e) => e.message).join(", ") || "invalid input";
    throw {
      status: 400,
